...
def liberdus_env_config() -> Optional[dict[str, Any]]:
    ...
    api_url = (os.getenv("LIBERDUS_API_URL") or "").strip() or None
...
class LiberdusAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._accounts = [a for a in (accounts or []) if isinstance(a, dict)]
    ...
    def _event_to_message(self, event: dict[str, Any]) -> MessageEvent | None:
        ...
        if profile not in _ALLOWED_PROFILES or policy_mode not in _ALLOWED_POLICY_MODES:
            return None
